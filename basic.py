...
class SubscriptNode:
    ...
    def __repr__(self):
        return f"{self.node_to_subscript}[{self.subscript_node}]"
...
class Parser:
    ...
    def subscript(self):
        ...
        if self.current_tok.type == TT_LSQUARE:
            ...
            return res.success(SubscriptNode(atom, subscript_node))
    ...
    def if_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, KW_TABLE["cond_once"]):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'if'"
            ))
        ...
        while self.current_tok.matches(TT_KEYWORD, KW_TABLE["cond_sec"]):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        elif tok.type == TT_ATTRIBUTE:
            res.register_advancement()
            self.advance()
            return res.success(AttrAccessNode(tok))
...
builtins_symbol_table.set("NULL", Number(0))
builtins_symbol_table.set("TRUE", Number(1))
builtins_symbol_table.set("FALSE", Number(0))

global_symbol_table = SymbolTable()
global_symbol_table.parent = builtins_symbol_table


def run(fn, text, max_stage="interpreter"):
    # Generate tokens
    lexer = Lexer(fn, text)
    ...
    if max_stage == "interpreter":
        if result is not None:
            return result.value, result.error
        else:
            return "", None
    else:
        raise ValueError("Invalid maximum stage")
